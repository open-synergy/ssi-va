...
odoo.define("ssi_va.va_biller_merchant_tour", function (require) {
    "use strict";

    var tour = require("web_tour.tour");

    // Shared navigation block reused by every tour below -- it corresponds to
    // Flow 1 of every va_biller_merchant IK: "Open the Financial Accounting >
    // Configuration > Virtual Account > Biller Merchants menu."
    //
    ...
    // for it.
    function openVABillerMerchantList() {
        return [
            ...
            {
                content: "Open the Biller Merchants menu",
                trigger:
                    '.o_menu_sections [data-menu-xmlid="ssi_va.va_biller_merchant_menu"]',
            },
            {
                ...
                content: "Virtual Account Biller Merchants list is displayed",
                trigger:
                    ".o_control_panel .breadcrumb-item.active:contains(Virtual Account Biller Merchants)",
    ...
    tour.register(
        "ssi_va_va_biller_merchant_create",
        {
            test: true,
            url: "/web",
        },
        [].concat(
            ...
            // Virtual Account > Biller Merchants menu.
            openVABillerMerchantList(),
            [
                ...
                // that is still "/".
                {
                    ...
                    run: "text TOUR VA MERCHANT Create",
                },
                {
                ...
                // ── Flow 5 — On the Biller Codes tab, add a line with Biller
                // Bank Code and Merchant Code. One line is enough: the IK
                // repeats the same clicks for every extra biller-bank
                // combination.
                //
                // The Bank sub-bullet of this Flow step has no step of its
                // own: the IK states it is "automatically filled from the
                // selected Biller Bank Code and cannot be changed", so it
                // takes no user action. Whether the derived value is right is
                // a value check and belongs to the unit tests.
                {
                    content: "Open the Biller Codes tab",
                    trigger: ".o_notebook .nav-link:contains(Biller Codes)",
                },
                {
                    content: "Add a line",
                    trigger:
                        ".o_field_widget[name='biller_code_ids'] " +
                        ".o_field_x2many_list_row_add a",
                },
                {
                    content: "Select the Biller Bank Code",
                    trigger: ".o_selected_row .o_field_widget[name='biller_id'] input",
                    run: "text TOUR VA MERCHANT BILLER",
                },
                {
                    // Match the va_biller.code display_name built by its
                    // name_get ("Biller - Bank (Code)"). Matching the biller
                    // part alone would also match the "Create ..." entry of
                    // the dropdown, which carries the typed text.
                    content: "Pick the biller bank code from the dropdown",
                    trigger:
                        ".ui-autocomplete .ui-menu-item " +
                        "a:contains(TOUR VA MERCHANT BILLER - TOUR VA MERCHANT BANK)",
                    in_modal: false,
                },
                {
                    content: "Fill in the Merchant Code",
                    trigger: ".o_selected_row .o_field_widget[name='merchant_code']",
                    run: "text TOURMCCREATE01",
                },
                {
                    ...
                    content: "Commit the biller code line",
                    trigger: ".o_field_widget[name='name']",
                    run: function () {
                        this.$anchor[0].click();
                    },
                ...
                // ── Post-Condition — A new Virtual Account Biller Merchant
                // record is created; the Code now holds a generated value and
                // the biller code line is attached to it. Only what is visible
                // is asserted here: that the record is saved and displayed,
                // and that the biller code line is rendered on its tab. The
                // generated Code value and the existence of the underlying
                // va_biller_merchant.code row -- including its derived Bank --
                // are value/database facts and belong to the unit tests.
                {
                    content: "Record is saved and displayed",
                    trigger:
                        ".o_control_panel .breadcrumb-item.active:contains(TOUR VA MERCHANT Create)",
                ...
                {
                    content: "Biller code line is shown on the Biller Codes tab",
                    trigger:
                        ".o_field_widget[name='biller_code_ids'] " +
                        ".o_data_row:contains(TOURMCCREATE01)",
                    run: function () {
                        // Assertion only; do not trigger the default click
                        // action.
                    },
    ...
    tour.register(
        "ssi_va_va_biller_merchant_edit",
        {
            test: true,
            url: "/web",
        },
        [].concat(
            // ── Flow 1 — Open the Biller Merchants menu.
            openVABillerMerchantList(),
            [
                // ── Flow 2 — Find and open the record to edit.
                {
                    content: "Open the biller merchant record",
                    trigger:
                        ".o_data_row:contains(TOUR VA MERCHANT Edit) .o_data_cell:first",
                    extra_trigger: ".o_list_view",
                },
                {
                ...
                // ── Flow 3 — Change the required fields.
                {
                    ...
                    trigger: ".o_field_widget[name='name']",
                    run: "text TOUR VA MERCHANT Edit Changed",
                },
                ...
                },
                {
                    ...
                    content: "Record is saved by Generate Code",
                    trigger:
                        ".o_control_panel .breadcrumb-item.active:contains(TOUR VA MERCHANT Edit Changed)",
                    run: function () {
                        // Assertion only; do not trigger the default click
                        // action.
                    },
                ...
                // ── Flow 5 — On the Biller Codes tab, add a line. Bank again
                // has no step of its own: the IK fills it automatically from
                ...
                // values and the biller code line is reflected on the tab. The
                // stored field values themselves, Bank included, are unit test
                // territory.
                {
                ...
                },
                {
                    ...
                    trigger:
                        ".o_field_widget[name='biller_code_ids'] " +
                        ".o_data_row:contains(TOURMCEDIT01)",
                    run: function () {
                        // Assertion only; do not trigger the default click
                        // action.
                    },
    ...
    tour.register(
        "ssi_va_va_biller_merchant_delete",
        {
            test: true,
            url: "/web",
        },
        [].concat(
            ...
            [
                // ── Flow 2 — Select the record to delete (checkbox).
                {
                    ...
                    trigger:
                        ".o_data_row:contains(TOUR VA MERCHANT Delete) " +
                ...
                // removed and no longer appears in the list. That its Biller
                ...
                // fact, verified by the unit tests, not by the tour.
                {
                    ...
                    trigger:
                        ".o_list_view:not(:has(.o_data_row:contains(TOUR VA MERCHANT Delete)))",
                    run: function () {
                        // Assertion only; do not trigger the default click
                        // action.
                    },
    ...
    tour.register(
        "ssi_va_va_biller_merchant_deactivate",
        {
            test: true,
            url: "/web",
        },
        [].concat(
            ...
            [
                // ── Flow 2 — Select the record to deactivate (checkbox).
                {
                    ...
                    trigger:
                        ".o_data_row:contains(TOUR VA MERCHANT Deactivate) " +
                        ".o_list_record_selector input",
                    run: "click",
                },
                ...
                // the IK Post-Condition (an archived merchant can no longer be
                // picked when generating Virtual Accounts, while existing data
                // keeps referencing it) are not visible facts of this screen
                // and belong to the unit tests.
                {
                    content: "Record no longer appears in the active list",
                    trigger:
                        ".o_list_view:not(:has(.o_data_row:contains(TOUR VA MERCHANT Deactivate)))",
                    run: function () {
                        // Assertion only; do not trigger the default click
                        // action.
                    },
    ...
    tour.register(
        "ssi_va_va_biller_merchant_activate",
        {
            test: true,
            url: "/web",
        },
        [].concat(
            ...
            [
                ...
                // ── Flow 3 — Select the record to reactivate (checkbox).
                {
                    ...
                    trigger:
                        ".o_data_row:contains(TOUR VA MERCHANT Activate) " +
                        ".o_list_record_selector input",
                    run: "click",
                },
                ...
                // docs/va_biller_merchant/05-activate.md; changing IK files is
                ...
                // to true.
                {
                    ...
                    trigger:
                        ".o_list_view:not(:has(.o_data_row:contains(TOUR VA MERCHANT Activate)))",
                    run: function () {
                        // Assertion only; do not trigger the default click
                        // action.
                    },
